...
class VdataControllerVdata extends VdataController
{
   ...
   function __construct()
	...
	{ 
		...
		header('Cache-Control: no-cache');
		 ...
		 require_once JPATH_ADMINISTRATOR.'/components/com_vdata/classes/oprating_lib.php';
		 ...
		 if (defined('PSI_JSON_ISSUE') && (PSI_JSON_ISSUE)) {
			...
			$cpu = new stdClass();
			$cpu->cpu_info = isset($json->Hardware->CPU->CpuCore[0]->attributes->Model)?$json->Hardware->CPU->CpuCore[0]->attributes->Model.' Cpu Speed'.$json->Hardware->CPU->CpuCore[0]->attributes->CpuSpeed:isset($json->Hardware->CPU->CpuCore->attributes->CpuSpeed)?$json->Hardware->CPU->CpuCore->attributes->CpuSpeed:'';
		    ...
		    for($j=0;$j<count($this->profiles);$j++){
			 ...
			 {      
 				   ...
 				   else
				   {
					...
					{
					$single_data = $single_data->data;
					$lsting = '<div class="listing_layout_others" style="height:'.($row_widget_height_value_chart-40).'px"><table class="adminlist table table-hover listing_info listing_info_'.$this->profiles[$j]->id.' profile_mid_data_inner" width="100%"><thead><tr>';
	...
	$html .= '<ul class="profile_mid_data_inner"'.$profile_style.'>';
	...
	$html .= '<ul class="profile_mid_data_inner">';
		 ...
		 $html .= '</ul>'; 	
		   ...
		   if($this->profile->datatype_option=='profile'){
			  $li  .= ' profile_widget';}
			   $li .= $box_class_name.'" data-ordering-profile="'.$this->profile->id.':'.$this->profile->ordering.'"><div class="panel_header"'.$style.'>'; 
			$html .= '<div class="panel_header">';
			if(isset($this->profile->name)&& $this->profile->name!='')
			$html .= '<span class="profile_name">'.$this->profile->name.'</span>';
			$html .= '<span class="delete-widget" onclick="delete_widget(this);" data-widget-id="'.$this->profile->id.'"><i class="icon-delete"></i></span>
   ...
   elseif($this->profile->datatype_option=='writequery'){
	...
	function update_dashboard(){
		...
		require(JPATH_SITE.'/components/com_vdata/views/vdata/tmpl/default.php');
		require_once JPATH_COMPONENT.'/operating/drawchart.php';
	...
	function live_chart_data()
	{
	   ...
	   if (defined('PSI_JSON_ISSUE') && (PSI_JSON_ISSUE)) {
		...
		$data->result = 'success';
		jexit(json_encode($data));
         /* $host =  'www.okhlites.com';//$config->get( 'sitename' );
		 $port = 80;
		 $timeout = 12;
		...
		$time_taken = $end_time - $start_time;
		$time_taken = round($time_taken,5);
       //$output = system('nslookup www.okhlites.com');print_r($output);
		
		       $html = $time_taken * 1000;
	   ...
	   if (defined('PSI_JSON_ISSUE') && (PSI_JSON_ISSUE)) {
		...
		$process_list = $db->setQuery('SHOW FULL PROCESSLIST')->loadObjectList();
		$procc = '';
		for($i=0;$i<count($process_list);$i++){
		$row = $process_list[$i];
		$procc .= '<tr><td>'.$row->Info.'</td><td>'.$row->Command.'</td><td>'.$row->Time.'</td></tr>';	
			
		}
		$data->json = str_replace('@attributes','attributes',$json);
		$data->html = $html;
		$data->info_cpu = $load;
		$data->info_memory = $mem_usage; 
		$data->procc = $procc;
		$data->result = 'success';
		jexit(json_encode($data)); */
	}
