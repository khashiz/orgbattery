...
class XML
{
        ...
        if (($procss = $this->_sys->getProcesses()) !== null) {
            ...
            if (isset($procss['*']) && (($procall = $procss['*']) > 0)) {
                ...
                if (!isset($procss[' ']) || !($procss[' '] > 0)) { // not unknown
					...
					if (isset($procss['name'])) {
						 $n_process = array();
						
						 for($f=0;$f<count($procss['name']);$f++){
							array_push($n_process,$procss['name'][$f]);
						 }
                        $vitals->addAttribute('ProcessesName', "{'".implode(',',$n_process)."'}"); 
    ...
    private function _buildXml()
    {
        ...
        $this->_buildPlugins();
		print_r($this->_sysinfo->getEncoding());//jexit();
        //$this->_xml->combinexml();
    }
