...
Joomla.submitbutton = function(task) {
	if (task == 'close' || task == 'close_st') {
		...
		function batchExport(offset){
				var values, index;
				values = $hd("#adminForm").serializeArray();//console.log(values);
				addOption('task', 'export_start', values);
				addOption('offset', offset, values);
				values = jQuery.param(values);//console.log(values);
...
<div class="col100">
	<fieldset class="adminform">
		<legend><?php echo JText::_( 'ATOM_DETAILS' ); ?></legend>
        <table class="adminform table table-striped adminformlist">
		<tr>
			<td width="200"><label class="hasTip" title="<?php echo JText::_('ATOM_ENTRY_TITLE_DESC');?>"><?php echo JText::_('ATOM_ENTRY_TITLE');?></label></td>
			<td>
		...
		<tr>
			<td width="200"><label class="hasTip" title="<?php echo JText::_('ATOM_ENTRY_LINK_DESC');?>"><?php echo JText::_('ATOM_ENTRY_LINK');?></label></td>
			<td>
		...
		<tr>
			<td width="200"><label class="hasTip" title="<?php echo JText::_('ATOM_ENTRY_SUMMARY_DESC');?>"><?php echo JText::_('ATOM_ENTRY_SUMMARY');?></label></td>
			<td>
				<select name="field[summary]">
					<option value=""><?php echo JText::_('SELECT_COLUMN');?></option>
					<?php foreach($options as $option){?>
						<option value="<?php echo $option;?>" <?php if( !empty($schedule_columns) && property_exists($schedule_columns->fields, 'summary') && ($schedule_columns->fields->summary==$option) ) echo 'selected="selected"';?>><?php echo $option;?></option>
					<?php }?>
				</select>
			</td>
		</tr>
		<tr>
			<td width="200"><label class="hasTip" title="<?php echo JText::_('ATOM_ENTRY_UPDATED_DESC');?>"><?php echo JText::_('ATOM_ENTRY_UPDATED');?></label></td>
			<td>
				<select name="field[updated]">
					<option value=""><?php echo JText::_('SELECT_COLUMN');?></option>
					<?php foreach($options as $option){?>
						<option value="<?php echo $option;?>" <?php if( !empty($schedule_columns) && property_exists($schedule_columns->fields, 'updated') && ($schedule_columns->fields->updated==$option) ) echo 'selected="selected"';?>><?php echo $option;?></option>
					<?php }?>
				</select>
			</td>
		</tr>
		<tr>
			<td width="200"><label class="hasTip" title="<?php echo JText::_('ATOM_ENTRY_ID_DESC');?>"><?php echo JText::_('ATOM_ENTRY_ID');?></label></td>
			<td>
				<select name="field[id]">
					<option value=""><?php echo JText::_('SELECT_COLUMN');?></option>
					<?php foreach($options as $option){?>
						<option value="<?php echo $option;?>" <?php if( !empty($schedule_columns) && property_exists($schedule_columns->fields, 'id') && ($schedule_columns->fields->id==$option) ) echo 'selected="selected"';?>><?php echo $option;?></option>
					<?php }?>
				</select>
			</td>
		</tr>
