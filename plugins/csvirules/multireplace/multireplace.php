...
class PlgCsvirulesMultireplace extends RantaiPluginDispatcher
{
	...
	{
		return array('value' => $this->id, 'text' => 'RO CSVI Multireplace');
	}
	...
	private function findreplace($value, $replacement)
	{
		...
		$stopFirst   = isset($replacement->stopfirstfind) ? $replacement->stopfirstfind : '';
			...
			switch ($replacement->method)
			{
				case 'text':
					...
					else
					{
						...
						if ($stopFirst)
						{
							foreach ($findText as $key => $find)
							{
								if (!in_array($find, $replacedValue))
								{
									$value           = str_replace($find, $replaceText[$key], $value);
									$replacedValue[] = $find;
								}
							}
						}
						...
						{
							$value = str_ireplace($findText, $replaceText, $value, $count);
						}
					}
