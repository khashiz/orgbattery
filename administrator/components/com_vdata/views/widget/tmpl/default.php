...
window.data_range_limit_value = function(limit_range_value){
	if(jQuery(limit_range_value).val()!='' && jQuery(limit_range_value).val()<=0){
	jQuery(limit_range_value).val(1);
	}
};	
window.user_write_query_value = function(e){
	                            /* if(jQuery(e).val()!=''){
...
window.update_autoselect_field = function(){
	...
	jQuery(function()	{
var table_column='';
var database_prefix = '<?php echo $config->get( 'db' );?>';
...
tips = jQuery( ".validateTips" );
	...
	function axis_val(){
		...
		var respose = '';var selected =''
		    ...
		    jQuery('select.series_col_change').chosen({"disable_search_threshold":0,"allow_single_deselect":true,"placeholder_text_multiple":"Select some options","placeholder_text_single":"Select an option","no_results_text":"No results match"});
		}
	...
	{ 
		...
		open: function(event, ui)
		   {
				...
				.html('<span class="ui-button-icon-primary ui-icon ui-icon-closethick"></span><span class="ui-button-text"></span>');
                    jQuery('.hasTipAseries').each(function() {
					  ...
					  beforeSend: function()	{
						jQuery("#vdata_overlay_widget_loading").show();
					  },
					  complete: function()	{
						jQuery("#vdata_overlay_widget_loading").hide();
					  },
	 ...
	 jQuery('body').on('click', 'span.select_style_for_series', function(event){
		 ...
		 {
			 alert("<?php echo JText::_('COM_VDATA_WIDGET_ADD_SERIES_FIRST');?>");
	        return;
		  }
 ...
 var valid_auto = false;
		...
		open: function(event, ui){
		   ...
		   if(jQuery('textarea[name="params[user_write_query_value]"]').length>0&& jQuery('textarea[name="params[user_write_query_value]"]').val()!='')
			  ...
			  else if(jQuery('select[name="params[existing_database_table]"]').val()!='' && jQuery('select[name="chart_type"]').val()=='Pie Chart' || jQuery('select[name="chart_type"]').val()=='Slice Pie Chart'){
				
			  jQuery.ajax({
		                url: "index.php",
						type: "POST",
						dataType: "json",
						data: {'option':'com_vdata', 'view':'widget', 'task':'series_color_object','table':jQuery('select[name="params[existing_database_table]"]').val(), 'chart_type':jQuery('select[name="chart_type"]').val(), 'extra':jQuery('textarea[name="params[extra_condition]"]').val(), 'limit':jQuery('input[name="params[limit_value]"]').val(), 'orderby':jQuery('select[name="params[ordering_reference_column_name]"]').val(), 'ordering':jQuery('select[name="params[ordering]"]').val(), 'column':jQuery('select[name="params[label_column_name]"]').val(), 'id':<?php echo isset($this->widget->id)&& $this->widget->id>0?$this->widget->id:0; ?>, "<?php echo JSession::getFormToken(); ?>":1, 'abase':1},
			...
			else
			 {
						...
						if(widget_chart=='Line Chart' || widget_chart == 'Area Chart'|| widget_chart == 'Stepped AreaChart'||widget_chart == 'Column Chart'|| widget_chart =='Pie Chart' || widget_chart =='Slice Pie Chart'  || widget_chart == 'Combo Chart' || widget_chart == 'Bar Chart'||  widget_chart =='Table Chart')
						{
						if(document.getElementById('series_name').value!='')
		...
		function addColor()
		{
			var s_v_e = c_v_e = new Array();
			 if(jQuery('textarea[name="params[user_write_query_value]"]').length>0&& jQuery('textarea[name="params[user_write_query_value]"]').val()!='')
			  ...
			  else if(jQuery('select[name="chart_type"]').val()=='Pie Chart')
			 {
	...
	jQuery.ajax({
						...
						beforeSend: function()	{
							jQuery("#vdata_overlay_second").show();
							jQuery('.save').removeClass('disabled');
							},
							complete: function()	{
							jQuery("#vdata_overlay_second").hide();
							},
							success: function(res)	{
							if(res.result == "success"){
		...
		jQuery.ajax({
						...
						data: {'option':'com_vdata', 'view':'widget', 'task':'data_for_query','data_base_options':jQuery('input[type=radio][name="datatype_option"]:checked').val(), 'id':<?php echo isset($this->widget->id)&& $this->widget->id>0?$this->widget->id:0; ?>, "<?php echo JSession::getFormToken(); ?>":1, 'abase':1},
						 ...
						 beforeSend: function()	{
							...
							jQuery('tr.chart_type_select').hide();
							jQuery('#editor_section').hide();
							if(!jQuery(".style_charting_formate").is(':disabled'))
							...
							jQuery('#show_all_profile').on('click', function(){
							   ...
							   var user_write_query_value_focusin ='', user_write_query_value_focusout ='';
							jQuery('textarea#user_write_query_value').on("focusin", function() {
								jQuery('.save').addClass('disabled');
								user_write_query_value_focusin = jQuery(this).val();
							}); 
                            jQuery('textarea#user_write_query_value').on("focusout", function() {
								user_write_query_value_focusout = jQuery(this).val();
								if(user_write_query_value_focusin==user_write_query_value_focusout)
								jQuery('.save').removeClass('disabled');
							});							
	...
	._renderItem = function(ul, item) {
        ...
        return listItem;
							...
							jQuery('select.selectoptions_class').chosen({"disable_search_threshold":0,"search_contains": true,"allow_single_deselect":true,"placeholder_text_multiple":"Select some options","placeholder_text_single":"Select an option","no_results_text":"No results match"});
							...
							alert(res.error);
							var JTooltips = new Tips($$('span.tooltip_widget'), { maxTitleChars: 50, fixed: false});
							},
							error: function(jqXHR, textStatus, errorThrown)	{
							alert(textStatus);				  
							}	
...
function selectoptions_for_chart()
    {
     ...
     if(jQuery('input[type=radio][name="datatype_option"]:checked').val()=='predefined'){
		...
		jQuery.ajax({
						...
						beforeSend: function()	{
							jQuery("#vdata_overlay_third").show();
							},
							complete: function()	{
							jQuery("#vdata_overlay_third").hide();
							},
							success: function(res)	{
							if(res.result == "success"){
							...
							if(jQuery('input[type=radio][name="datatype_option"]:checked').val()=='writequery' && jQuery('textarea[name="params[user_write_query_value]"]').val()==''){
								...
								if(jQuery('input[type=radio][name="params[style_layout]"]:checked').val()=='charting_formate' && jQuery('select[name="chart_type"]').val()!='')
								{
								jQuery('.create_series').attr('disabled',false);
							...
							jQuery('select.selectoptions_class').chosen({"disable_search_threshold":0,"allow_single_deselect":true,"placeholder_text_multiple":"Select some options","placeholder_text_single":"Select an option","no_results_text":"No results match"});
...
window.callback=function(e,series_name_delet,series_name_column_delet,closest_li_event)
    { 
		 ...
		 var s_v_e, s_v_c = new Array();
		  var chart_type = jQuery('#chart_type').val();
						if(chart_type=='Area Chart' || chart_type=='Column Chart'  || chart_type=='Line Chart'  || chart_type=='Stepped AreaChart'  || chart_type=='Table Chart'  || chart_type=='Combo Chart'|| chart_type=='Bar Chart' || chart_type=='Bubble Chart')
						{
	...
	/* jQuery('body').on('focus','.hasDatepicker',function(){alert(12);
	jQuery(this).datetimepicker({dateFormat:'yy-mm-dd',timeFormat:'HH:mm:ss'});		  
		}); */
window.extra_formating_style =function(layout_style){
	
	 jQuery('select[name="chart_type"]').removeClass('required');
	 jQuery('select[name="chart_type"]').prop("selected", false)
	 jQuery('.selectoptions_class').trigger('liszt:updated');
	 if(jQuery('select[name="params[reference_listing_column]"]').length>0)
	 ...
	 extra_condition_dialog = jQuery( ".container_backhand" ).dialog({
		...
		open: function(event, ui){
			...
			if(jQuery('input[type=radio][name="datatype_option"]:checked').val() == 'predefined'){
				...
				else{var table_name = jQuery('select[name="params[existing_database_table]"]').val();}
			jQuery.ajax({
				  url: "index.php",
				  type: "POST",
				  dataType: "json",
...
Joomla.submitbutton = function(task) 
         {
			if (task == 'cancel') 
			{   
		        Joomla.submitbutton(task, document.getElementById('adminForm'));
...
<div id="vdatapanel">
<div id="vdata_overlay_widget_loading" class="vdata_overlay" style="display:none;"> 
<img class="vdata-loading" src="<?php echo JURI::root();?>media/com_vdata/images/loading_second.gif" alt="">
</div>
<div id="vdata_overlay_second" class="vdata_overlay_second" style="display:none;"> 
<img class="vdata-loading" src="<?php echo JURI::root();?>media/com_vdata/images/loading_second.gif" alt="">
</div>
<div id="vdata_overlay_third" class="vdata_overlay_third" style="display:none;"> 
<img class="vdata-loading" src="<?php echo JURI::root();?>media/com_vdata/images/loading_second.gif" alt="">
</div>
<div class="vdata_add_widget_buttons">
		   ...
		   <span class="widget_apply_section">
			...
			<label class="hasTip datatype-label drinkcard-cc predefined-chart" for="check1" title="<?php echo JText::_( 'COM_VDATA_WIDGET_CREATION_PREDEFINED_QUERY_DESC' ); ?>">
            </label>
			...
			<label class="hasTip datatype-label drinkcard-cc create-chart" for="check2" title="<?php echo JText::_( 'COM_VDATA_WIDGET_CREATION_MANUALLY_QUERY_DESC' ); ?>"></label>
			<span class="label-message">
            <?php echo JText::_( 'COM_VDATA_WIDGET_CREATION_MANUALLY_QUERY' ); ?></span></span>
			</div>
			<div class="vdat_button_section">
                <input type="submit" class="btn btn-success save" id="save" onclick="valforms(this);"  value="<?php echo JText::_('COM_WIDGET_SAVE'); ?>"/>
				<input type="submit" class="btn cancel btn-danger" id="cancel" onclick="valforms(this);" value="<?php echo JText::_('COM_WIDGET_CANCEL'); ?>"/></div>
				</div>
...
<div class="container_backhand" title="Put Extra Condition on Data" style="display:none"> 
