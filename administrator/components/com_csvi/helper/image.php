...
class CsviHelperImage
{
	...
	/**
	 * Holds the mime types it found
	 *
	 * @var array
	 * @since  3.0
	 */
	private $_found_mime_type = [];

	/**
	 * Contains all the image data for processing
	 *
	 * @var array
	 * @since  3.0
	 */
	...
	/**
	 * 0-255 - red color variable for background filler
	 *
	 * @var int $bg_red
	 * @since  3.0
	 */
	...
	/**
	 * 0-255 - green color variable for background filler
	 *
	 * @var int $bg_green
	 * @since  3.0
	 */
	...
	/**
	 * 0-255 - blue color variable for background filler
	 *
	 * @var int $bg_blue
	 * @since  3.0
	 */
	...
	/**
	 * 0-1 - true/false - should thumbnail be filled to max pixels
	 *
	 * @var int $maxSize
	 * @since  3.0
	 */
	 ...
	 *
	 * @var string $file_out_extension
	 * @since  3.0
	 */
	public $file_out_extension = null;

	/**
	 * The width of the file to be generated
	 *
	 * @var int $file_out_width
	 * @since  3.0
	 */
	private $file_out_width = 0;

	/**
	 * The height of the file to be generated
	 *
	 * @var int $file_out_height
	 * @since  3.0
	 */
	 ...
	 * @since   3.0
	 */
	public function isImage($file, $remote = false, $encodeUrl = 1)
	{
		$mime_type = $this->findMimeType($file, $remote, $encodeUrl);
		...
		if ($mime_type)
		{
			...
			{
				if ($fileType['mime_type'] == $mime_type)
				{
					return true;
				}
			}
	 ...
	 * @since   3.0
	 */
	public function findMimeType($filename, $remote = false, $encodeUrl = 1)
	{
	...
	private function loadFileTypes()
	{
		...
		$this->fileTypes[0]['mime_type']  = 'application/pdf';
	}
	...
	public function processImage($name, $output_path, $output_name = null)
	{
		...
		if ($this->_imagedata['isremote'])
		{
			...
			if ($remoteMethod)
			{
				...
				if ($this->isImage($this->_imagedata['name'], true, $encodeURL))
				{
			...
			else
			{
				...
				$this->log->addStats('information', Text::sprintf('COM_CSVI_IMAGE_REMOTE_NOT_EXISTS', $this->_imagedata['name']));
				$this->_imagedata['exists'] = false;
			}
		}
		...
		{
			// File does not exist
			$this->log->add(Text::sprintf('COM_CSVI_DEBUG_FILE_NOT_FOUND', $this->_imagedata['name']), false);
			$this->log->addStats(
				'incorrect',
				Text::sprintf('COM_CSVI_DEBUG_FILE_NOT_FOUND', $this->_imagedata['name'])
			);
		...
		if ($this->_imagedata['isimage'])
		{
			...
			if ($this->_imagedata['isremote'] && $this->template->get(
					'save_images_on_server',
					'image'
				) && !$this->_imagedata['convert'])
			{
				// Sanitize filename
				...
				else
				{
					if ($from !== $to)
					{
						...
						// If the given folder path is not found then create it
						if (!JFolder::exists($this->_imagedata['base'] . '/' . $this->_imagedata['output_path']))
						{
							JFolder::create($this->_imagedata['base'] . '/' . $this->_imagedata['output_path']);
						}

						if (!JFile::move($from, $to))
						{
							$this->log->add('Folder location not found ' . $to, false);
							$this->log->addStats('incorrect', Text::sprintf('COM_CSVI_FOLDER_LOCATION_NOT_FOUND', $to));
						}
					}
				}
			}
		...
		else
		{
			if ($this->_imagedata['exists'])
			{
				...
				$this->_imagedata['output_name'] = JFile::stripExt(
						$this->_imagedata['output_name']
					) . '.' . $this->_imagedata['extension'];
		...
		// Finally after all processing set isimage as false if it is a pdf file
		if ($this->_imagedata['extension'] === 'pdf')
		{
			$this->_imagedata['isimage'] = false;
		}
	...
	public function createThumbnail($original, $output_path, $output_name)
	{
		...
		if ($this->template->get('thumb_check_filetype', 'image') && JFile::getExt($original) != JFile::getExt(
				$output_name
			))
		{
		...
		if ($this->template->get('thumb_width') >= 1 && $this->template->get('thumb_height') >= 1)
		{
			...
			if ($file_exists)
			{
				...
				if ($original != $thumb_file_details['file_out'])
				{
					...
					if ($new_img)
					{
						...
						{
							$this->log->add(
								'Thumbnail has not been created because the file ' . $new_img . ' does not exist',
								false
							);
			...
			else
			{
				...
				$this->log->AddStats(
					'nofiles',
					Text::sprintf('COM_CSVI_FILE_DOES_NOT_EXIST_NOTHING_TO_DO', $original)
				);
	...
	private function cleanupImage()
	{
		...
		if ($this->_imagedata['isremote'] && $this->template->get('save_images_on_server', 'image'))
		{
			...
			if (!JFile::exists($curfile) || (JFile::exists($curfile) && $this->template->get(
						'redownload_external_image',
						'image'
					)))
			{
				...
				if ($this->storeRemoteImage(
					$this->_imagedata['name'],
					$local_image,
					$this->template->get('image_encodeurl', 1)
				))
				{
				...
				else
				{
					$this->log->AddStats(
						'nofiles',
						Text::sprintf('COM_CSVI_REMOTE_FILE_NOT_FOUND', $this->_imagedata['name'])
					);
					$this->log->add(Text::sprintf('COM_CSVI_REMOTE_FILE_NOT_FOUND', $this->_imagedata['name']), false);
				}
			}
			else
			{
		...
		if ($ext != strtolower($output_ext))
		{
			...
			{
				$this->log->add(
					'Renaming full image because bad extension: ' . $this->_imagedata['name'] . ' --> ' . $to,
					false
				);
		...
		if ($convert_type !== 'none' && $convert_type !== $this->_imagedata['extension'])
		{
			...
			{
				$this->_imagedata['output_name'] = JFile::stripExt(
						basename($this->_imagedata['output_name'])
					) . '.' . $convert_type;
			}
			else
			{
				$this->_imagedata['output_name'] = JFile::stripExt(
						basename($this->_imagedata['name'])
					) . '.' . $convert_type;
			}
		...
		$this->log->add(
			'Full name target: ' . $this->_imagedata['output_path'] . $this->_imagedata['output_name'],
			false
		);
	}
	 ...
	 * @since   3.0
	 */
	private function storeRemoteImage($remoteImage, $local_image, $encodeUrl = 1)
	{
		// Fix spaces in the remote image
		$dirPath   = dirname($remoteImage);
		$imageName = basename($remoteImage);
		...
		{
			$user = $this->template->get('image_username', '');
			$pass = $this->template->get('image_password', '');
	...
	private function convertFullImage()
	{
		...
		if ($this->_imagedata['convert'] && ($this->_imagedata['isremote'] && $this->template->get(
					'save_images_on_server',
					'image'
				)))
		{
			...
			$this->log->add('Convert image from ' . $file_details['file'] . ' to ' . $file_details['file_out']);
			$new_img = $this->convertImage($file_details);

			if ($new_img)
			{
				$this->log->add('Image converted to ' . $file_details['file']);
	...
	private function renameImage()
	{
			...
			&& (basename($this->_imagedata['name']) !== $this->_imagedata['output_name']))
		{
			...
			{
				$this->log->add(Text::sprintf('COM_CSVI_RENAME_FULL_FILE_NOT_FOUND', $from), false);
			}
		}
	}
	...
	private function imageTypeCheck()
	{
		...
		if (!$this->_imagedata['isremote'] && JFile::exists($this->_imagedata['name']) && !stristr(
				$this->_imagedata['mime_type'],
				$mime_ext
			))
		{
			$file_details                       = [];
	...
	private function resizeFullImage()
	{
		...
		if ($this->template->get('full_resize', 'image'))
		{
			...
			if (JFile::exists($checkfile))
			{
				...
				if ($cur_size[0] > $this->template->get('full_width', 'image') || $cur_size[1] > $this->template->get(
						'full_height',
						'image'
					))
				{
					...
					$this->log->add(
						sprintf(
							...
							$file_details['file'],
							$cur_size[1] . 'x' . $cur_size[0],
							$this->template->get('full_height') . 'x' . $this->template->get('full_width')
						)
					);
