...
final class CsviHelperImportfields extends CsviHelperFields
{
	...
	/**
	 * Initialise an import to call in another
	 *
	 * @param   string  $extension  Extension name of import
	 * @param   string  $component  Component name of import
	 * @param   string  $operation  Operation of import
	 *
	 * @return  object import model.
	 *
	 * @since   7.15.0
	 */
	public function initialiseAnotherImport($extension, $component, $operation)
	{
		if (!JFile::exists(JPATH_PLUGINS . '/csviaddon/' . $extension . '/' . $component . '/model/import/' . $operation . '.php'))
		{
			throw new InvalidArgumentException(JText::sprintf('COM_CSVI_ADDON_OPERATION_DOES_NOT_EXIST', $operation, $extension, $component));
		}
		...
		require_once JPATH_PLUGINS . '/csviaddon/' . $extension . '/' . $component . '/helper/' . $component . '.php';
		$csviHelper          = new CsviHelperCsvi;
		$componentHelpername = $component . 'Helper' . $component;
		$componentHelper     = new $componentHelpername($this->template, $this->log, $this, $this->db);
