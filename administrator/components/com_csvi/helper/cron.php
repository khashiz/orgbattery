...
class Csvicron extends JApplicationCli
{
	...
	private function loadTemplateId()
	{
		...
		if ($template_id || $template_name || $templateAlias)
		{
			if (empty($template_id) && ($template_name || $templateAlias))
			{
				...
				if ($template_name)
				{
					$query->where($this->db->quoteName('template_name') . ' = ' . $this->db->quote($template_name));
				}
				else
				{
					$query->where($this->db->quoteName('template_alias') . ' = ' . $this->db->quote($templateAlias));
				}
