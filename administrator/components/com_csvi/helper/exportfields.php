...
final class CsviHelperExportfields extends CsviHelperFields
{
	...
	public function getDateFormat($fieldName, $fieldValue, $columnHeader = '')
	{
		...
		$finalValue   = JHtml::_('date', $fieldValue, $this->template->get('export_date_format'), null);

		if ($fieldDetails->field_date_format)
		{
			$finalValue = JHtml::_('date', $fieldValue, $fieldDetails->field_date_format, null);
		}
	...
	/**
	 * Get the decimal separator from template
	 *
	 * @return  array  Separator from template
	 *
	 * @since   7.15.0
	 */
	...
	{
		$separator                       = [];
		$separator['decimalseparator']   = $this->template->get('export_price_format_decsep');
		$separator['thousandsseparator'] = $this->template->get('export_price_format_thousep');
