...
class CsviHelperFileImportCsv extends CsviHelperFile
{
	...
	private function findDelimiters($force = false)
	{
		if (!$this->checked_delimiter)
		{
			if (!$force && !$this->template->get('auto_detect_delimiters', true))
			{
				...
				$this->text_enclosure = $this->template->get('text_enclosure', '') ? $this->template->get('text_enclosure') : '';
			}
			else
			{
				// Read the first line
	...
	{
		if ($this->text_enclosure
			&& strpos($data, $this->text_enclosure) === 0
			&& strrpos($data, $this->text_enclosure) === 0
		)
		{
