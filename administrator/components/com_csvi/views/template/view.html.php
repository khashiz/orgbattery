...
class CsviViewTemplate extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		// Check if we get a call back from Google
		try
		{
			$clientId            = $this->item->options->get('clientid');
			$clientSecret        = $this->item->options->get('clientsecret');
			$googleHelper        = new CsviHelperGoogle($clientId, $clientSecret, $this->item->csvi_template_id);
			$existingAccessToken = new Registry($this->item->options->get('fulltoken'));

			if ($code = $jinput->get('code', '', 'RAW'))
			{
				$accessToken = $googleHelper->getAccessToken($code, $existingAccessToken->toArray());
				$this->item->options->set('fulltoken', json_encode($accessToken));

				if (isset($accessToken['access_token']))
				{
					$this->accesstoken = $accessToken['access_token'];
					$this->item->options->set('accesstoken', $this->accesstoken);

				}
			}
			else
			{
				if ($clientId && $clientSecret && $this->item->csvi_template_id)
				{
					$checkAccessToken = $googleHelper->checkAccessToken($existingAccessToken->toArray());
					$this->item->options->set('fulltoken', $existingAccessToken->toString());
		...
		{
			JFactory::getApplication()->enqueueMessage($exception->getMessage());
		}
	...
	private function addToolbar()
	{
		...
		else
		{
			...
			JToolbarHelper::custom('template.runTemplate', 'play', 'play', JText::_('COM_CSVI_RUNTEMPLATE_LABEL'), false);
