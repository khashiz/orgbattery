		...
		<form action="index.php?option=com_csvi&view=importsource" id="adminForm" name="adminForm" method="post" class="form-horizontal" enctype="multipart/form-data">
			...
			switch ($this->template->get('source', 'fromupload'))
			{
				...
				case 'fromtextfield':
					...
					break;
				case 'fromdatabase':
					if ($this->template->get('databasetype') === 'local')
					{
						echo JText::sprintf('COM_CSVI_IMPORT_FROM_LOCAL_DATABSE_TABLE', $this->template->get('localtablelist'));
					}
					else
					{
				...
				case 'fromgooglesheet':
					echo JText::_('COM_CSVI_IMPORT_FROM_GOOGLE_SHEET_LABEL');
					break;
			}
