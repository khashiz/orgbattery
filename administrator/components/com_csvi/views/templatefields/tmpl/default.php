...
// If there is join tables then show the table name and table alias columns
$singleTable = true;

if ($this->action === 'export' && $this->customTable)
{
	$customTablesArray = json_decode(json_encode($this->customTable), true);

	if (count($customTablesArray) > 2)
	{
		$singleTable = false;
	}
}
					...
					<tr>
						...
						<?php
							if ($this->action === 'export')
							{
								...
								?>
									<th>
										<?php echo JText::_('COM_CSVI_TABLE_NAME_LABEL'); ?>
									</th>
									<th>
										<?php echo JText::_('COM_CSVI_TABLE_ALIAS_LABEL'); ?>
									</th>
								<?php
								}
								?>
				...
				<?php
					...
					?>
						<tr class="sortable-group-id="<?php echo $item->csvi_template_id; ?>">
							...
							<?php if (!$singleTable && $this->action === 'export')
							{
								?>
								<td>
