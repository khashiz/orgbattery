...
class CsviModelMaintenance extends CsviModelDefault
{
	...
	public function runOperation($component, $operation, $key, $isCli = false)
	{
		...
		if ($addon = $this->loadAddon($component, $isCli))
		{
			...
			{
				throw new CsviException(JText::sprintf('COM_CSVI_OPERATION_NOT_EXIST', $operation, $component), 408);
			}
	...
	public function checkAddonVersion()
	{
		...
		$dotPosition = strpos($extensionManifest['version'], '.', 2);
		$extensionVersion = substr($extensionManifest['version'], 0, $dotPosition);
		...
		if (!$showMessage)
		{
			...
			foreach ($addons as $addon)
			{
				...
				$dotPosition = strpos($extensionManifest['version'], '.', 2);
				$addonVersion  = substr($manifestCache['version'], 0, $dotPosition);
		...
		if ($showMessage && $this->settings->get('enableaddonversioncheck', true))
		{
			JFactory::getApplication()->enqueueMessage($showMessage, 'error');
		}
	}
	...
	{
		return ['virtuemart','hikashop', 'k2', 'form2content', 'rsform',
			'productbuilder', 'fastseller', 'customfilters', 'j2store'];
	}
}
